...
def internal_force_elem(X, T, ie, gp, Wgauss, saved_stress_xx,saved_stress_yy,saved_stress_xy,thickness,mid_lay,pos):
        ...
        while jj<nPlies:
            ...
            while thick<3:
                ...
                while ii < Wgauss.shape[0] :
                     ...
                     if thick !=1:
                        qe += Wgauss[ii] * np.linalg.det(J) * np.dot(B.T, stress) * thickness[jj][0]/6
                     else:
                        qe += 4 * Wgauss[ii] * np.linalg.det(J) * np.dot(B.T, stress) * thickness[jj][0] / 6
