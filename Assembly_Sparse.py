...
def Assembly2D(X,T,f,g,h,Wgauss,gp,Ngauss,Klaw,pho,thickness,analysis_type):
    ...
    while ie < Nt :
     ...
     Fe = sp.lil_matrix(SMelem(f, g, h, X, T, ie, Ngauss, Wgauss))
...
def BCAssembly(X,T,b,F,j,fixedx):
    ...
    FBC=sp.lil_matrix((b.size,1))
    ie=0
    while ie<b.size-1:
        FEBC=sp.lil_matrix(SMBC(F,X,T,ie,b,j,fixedx))
        for ii in [1,2]:
            I=ie+ii-1
            FBC[I,0]+=FEBC[ii-1,0]
...
def BCMAssembly(X,T,b,F,j,fixedx,xy):
    ...
    FBCM=sp.lil_matrix((b.size,1))
    ie=0
    while ie<b.size-1:
        FEBCM=sp.lil_matrix(SMBCM(F,X,T,ie,b,j,fixedx,xy))
        for ii in [1,2]:
            I=ie+ii-1
...
def k_calc(X,T,w,thetax,thetay,ie):
    ...
    for ii in [0,1,2]:
        ...
        ddP[:, 0] = np.array([[0,0,0,0,0,0,2 * L[1] + L[1] * L[2] * 3 * (1 - mu[0,2]),L[1] * L[2] * (1 + 3 * mu[0,0]),-L[1] * L[2] * (1 + 3 * mu[0,1])]])

        ddP[:, 1] = np.array([[0,0,0,0,0,0,-L[0]*L[2]*(1+3*mu[0,2]) , 2*L[2]+L[0]*L[2]*3*(1-mu[0,0]) , L[0]*L[2]*(1+3*mu[0,1])]])

        ddP[:, 2] = np.array([[0,0,0,0,0,0, L[0]*L[1]*(1+3*mu[0,2]) , -L[0]*L[1]*(1+3*mu[0,0]) , 2*L[0]+L[0]*L[1]*3*(1-mu[0,1])]])

        ddP[:, 3] = np.array([[0,0,0,1,0,0,2*L[0]+L[0]*L[2]*3*(1-mu[0,2])-L[1]*L[2]*(1+3*mu[0,2])+0.5*L[2]**2*(1+3*mu[0,2]) , L[1]*L[2]*3*(1-mu[0,0])-0.5*L[2]**2*(1+3*mu[0,0])+L[0]*L[2]*(1+3*mu[0,0]), 0.5*L[2]**2*3*(1-mu[0,1])-L[0]*L[2]*(1+3*mu[0,1])+L[1]*L[2]*(1+3*mu[0,1])]])

        ddP[:, 4] = np.array([[0,0,0,0,0,1,L[0]*L[1]*3*(1-mu[0,2])-0.5*L[1]**2*(1+3*mu[0,2])+L[1]*L[2]*(1+3*mu[0,2]),0.5*L[1]**2*3*(1-mu[0,0])-L[1]*L[2]*(1+3*mu[0,0])+L[0]*L[1]*(1+3*mu[0,0]),2*L[2]+L[1]*L[2]*3*(1-mu[0,1])-L[0]*L[1]*(1+3*mu[0,1])+0.5*L[1]**2*(1+3*mu[0,1])]])

        ddP[:, 5] = np.array([[0,0,0,0,1,0,0.5*L[0]**2*3*(1-mu[0,2])-L[0]*L[1]*(1+3*mu[0,2])+L[0]*L[2]*(1+3*mu[0,2]), 2*L[1]+L[0]*L[1]*3*(1-mu[0,0])-L[0]*L[2]*(1+3*mu[0,0])+0.5*L[0]**2*(1+3*mu[0,0]), L[0]*L[2]*3*(1-mu[0,1])-0.5*L[0]**2*(1+3*mu[0,1])+L[0]+L[1]*(1+3*mu[0,1])]])

        ddN11 = np.array([[ddP[0, 0] - ddP[3, 0] + ddP[5, 0] + 2 * (ddP[6, 0] - ddP[8, 0]), ddP[0, 3] - ddP[3,3] + ddP[5, 3] + 2 * (ddP[6, 3] - ddP[8, 3]) , ddP[0, 4] - ddP[3 , 4] + ddP[5, 4] + 2 * (ddP[6, 4] - ddP[8 , 4])],
                          [ddP[0, 3] - ddP[3, 3] + ddP[5, 3] + 2 * (ddP[6, 3] - ddP[8, 3]), ddP[0, 1] - ddP[3,1] + ddP[5, 1] + 2 * (ddP[6, 1] - ddP[8, 1]), ddP[0, 5] - ddP[3, 5] + ddP[5, 5] + 2 * (ddP[6, 5] - ddP[8, 5])],
                          [ddP[0, 4] - ddP[3, 4] + ddP[5, 4] + 2 * (ddP[6, 4] - ddP[8, 4]), ddP[0, 5] - ddP[3,5] + ddP[5, 5] + 2 * (ddP[6, 5] - ddP[8, 5]), ddP[0, 2] - ddP[3, 2] + ddP[5, 2] + 2 * (ddP[6, 2] - ddP[8, 2])]])
        ...
        k[np.arange(3 * ii + 1, 3 * ii + 4)] = -np.array([[w[T[ie, 0]] * ddN11[0, 0] + thetax[T[ie, 0]] * ddN12[0, 0] + thetay[T[ie, 0]] * ddN13[0, 0] + w[T[ie, 1]] * ddN21[0, 0] + thetax[T[ie, 1]] * ddN22[0, 0] + thetay[T[ie, 1]] * ddN23[0, 0] + w[T[ie, 2]] * ddN31[0, 0] + thetax[T[ie, 2]] * ddN32[0, 0] + thetay[T[ie, 2]] * ddN33[0, 0]],
                    [w[T[ie, 1]] * ddN11[1,1] + thetax[T[ie,1]] * ddN12[1,1] + thetay[T[ie,1]] * ddN13[1,1] +         w[T[ie, 1]] * ddN21[1,1] + thetax[T[ie,1]] * ddN22[1,1] + thetay[T[ie,1]] * ddN23[1,1] + w[T[ie, 2]] * ddN31[1,1] + thetax[T[ie, 2]] * ddN32[1,1] + thetay(T[ie, 2]) * ddN33[1,1]],
                    [w[T[ie, 1]] * ddN11[0,1] + thetax[T[ie,1]] * ddN12[0,1] + thetay[T[ie,1]] * ddN13[0,1] +         w[T[ie, 1]] * ddN21[0,1] + thetax[T[ie,1]] * ddN22[0,1] + thetay[T[ie,1]] * ddN23[0,1] + w[T[ie, 2]] * ddN31[0,1] + thetax[T[ie, 2]] * ddN32[0,1] + thetay(T[ie, 2]) * ddN33[0,1]]]).T
...
def FEM(f,g,h,NX1,NY1,NX2,NY2,NX3,NY3,NX4,NY4,MY1,MXY1,MX2,MXY2,MY3,MXY3,MX4,MXY4,boundaryconditions,ENFRCDS,X,T,b,Ngauss,Klaw,box,pointload,NODALLOAD,pho,thickness,analysis_type,transient):
    ...
    if boundaryconditions[0] == 1:
      ...
      M[:, border1-1]=0
      F[border1-1,:] = 0
      K[np.ix_(border1-1, border1-1)] = sp.eye(border1.size)
      M[np.ix_(border1-1, border1-1)] = sp.eye(border1.size)
      ...
      for c in srch[0]:
       ...
       F[border1[np.arange(c,border1.size,6)]-1,:]=sp.lil_matrix(ENFRCDS1[0, c])
       ...
       K[np.ix_(border1[np.arange(c, border1.size, 6)] - 1, border1[np.arange(c, border1.size, 6)] - 1)] -= sp.diags((K[np.ix_(border1[np.arange(c,border1.size,6)]-1, border1[np.arange(c,border1.size,6)]-1)]).diagonal()) - sp.eye(border1[np.arange(c,border1.size,6)].size)
       M[np.ix_(border1[np.arange(c, border1.size, 6)] - 1, border1[np.arange(c, border1.size, 6)] - 1)] -= sp.diags((M[np.ix_(border1[np.arange(c,border1.size,6)]-1, border1[np.arange(c,border1.size,6)]-1)]).diagonal()) - sp.eye(border1[np.arange(c,border1.size,6)].size)
        ...
        M[:, border1[::6] - 1] = 0
        F[border1[::6] - 1,:] = 0
        K[np.ix_(border1[::6] - 1, border1[::6] - 1)] = sp.eye(border1[::6].size)
        M[np.ix_(border1[::6] - 1, border1[::6] - 1)] = sp.eye(border1[::6].size)
        ...
        M[:, border1[1::6] - 1] = 0
        F[border1[1::6] - 1,:] = 0
        K[np.ix_(border1[1::6] - 1, border1[1::6] - 1)] = sp.eye(border1[1::6].size)
        M[np.ix_(border1[1::6] - 1, border1[1::6] - 1)] = sp.eye(border1[1::6].size)
        ...
        M[:, border1[2::6] - 1] = 0
        F[border1[2::6] - 1,:] = 0
        K[np.ix_(border1[2::6] - 1, border1[2::6] - 1)] = sp.eye(border1[2::6].size)
        M[np.ix_(border1[2::6] - 1, border1[2::6] - 1)] = sp.eye(border1[2::6].size)
        ...
        M[:, border1[3::6] - 1] = 0
        F[border1[3::6] - 1,:] = 0
        K[np.ix_(border1[3::6] - 1, border1[3::6] - 1)] = sp.eye(border1[3::6].size)
        M[np.ix_(border1[3::6] - 1, border1[3::6] - 1)] = sp.eye(border1[3::6].size)
        ...
        M[:, border1[4::6] - 1] = 0
        F[border1[4::6] - 1,:] = 0
        K[np.ix_(border1[4::6] - 1, border1[4::6] - 1)] = sp.eye(border1[4::6].size)
        M[np.ix_(border1[4::6] - 1, border1[4::6] - 1)] = sp.eye(border1[4::6].size)
    ...
    if boundaryconditions[1] == 1:
      ...
      if boundaryconditions[0] == 2:
       ...
       K[np.ix_(border2[np.arange(0,6)]-1, border2[np.arange(0,6)]-1)] = sp.eye(6)
       M[np.ix_(border2[np.arange(0,6)]-1, border2[np.arange(0,6)]-1)] = sp.eye(6)
      ...
      M[:, border2-1] = 0
      F[border2-1,:] = 0
      K[np.ix_(border2-1, border2-1)] = sp.eye(border2.size)
      M[np.ix_(border2-1, border2-1)] = sp.eye(border2.size)
     ...
     if boundaryconditions[0] == 2:
      ...
      for c in srch[0]:
       ...
       K[np.ix_(border2[np.arange(c, border2.size, 6)] - 1, border2[np.arange(c, border2.size, 6)] - 1)] -= sp.diags((K[np.ix_(border2[np.arange(c, border2.size, 6)] - 1, border2[np.arange(c, border2.size, 6)] - 1)]).siagonal())  - sp.eye(border2[np.arange(c,border2.size,6)].size)
       M[border2[np.arange(c,border2.size,6)]-1,:]=0
       M[np.ix_(border2[np.arange(c, border2.size, 6)] - 1, border2[np.arange(c, border2.size, 6)] - 1)] -= sp.diags((K[np.ix_(border2[np.arange(c, border2.size, 6)] - 1, border2[np.arange(c, border2.size, 6)] - 1)]).siagonal())  - sp.eye(border2[np.arange(c,border2.size,6)].size)
        ...
        M[:, border2[::6] - 1] = 0
        F[border2[::6] - 1,:] = 0
        K[np.ix_(border2[::6] - 1, border2[::6] - 1)] = sp.eye(border2[::6].size)
        M[np.ix_(border2[::6] - 1, border2[::6] - 1)] = sp.eye(border2[::6].size)
        ...
        M[:, border2[1::6] - 1] = 0
        F[border2[1::6] - 1,:] = 0
        K[np.ix_(border2[1::6] - 1, border2[1::6] - 1)] = sp.eye(border2[1::6].size)
        M[np.ix_(border2[1::6] - 1, border2[1::6] - 1)] = sp.eye(border2[1::6].size)
        ...
        M[:, border2[2::6] - 1] = 0
        F[border2[2::6] - 1,:] = 0
        K[np.ix_(border2[2::6] - 1, border2[2::6] - 1)] = sp.eye(border2[2::6].size)
        M[np.ix_(border2[2::6] - 1, border2[2::6] - 1)] = sp.eye(border2[2::6].size)
        ...
        M[:, border2[4::6] - 1] = 0
        F[border2[4::6] - 1,:] = 0
        K[np.ix_(border2[4::6] - 1, border2[4::6] - 1)] = sp.eye(border2[4::6].size)
        M[np.ix_(border2[4::6] - 1, border2[4::6] - 1)] = sp.eye(border2[4::6].size)
        ...
        M[:, border2[3::6] - 1] = 0
        F[border2[3::6] - 1,:] = 0
        K[np.ix_(border2[3::6] - 1, border2[3::6] - 1)] = sp.eye(border2[3::6].size)
        M[np.ix_(border2[3::6] - 1, border2[3::6] - 1)] = sp.eye(border2[3::6].size)
    ...
    if boundaryconditions[2] == 1:
      ...
      if boundaryconditions[1] == 2:
       ...
       K[np.ix_(border3[np.arange(0,6)]-1, border3[np.arange(0,6)]-1)] = sp.eye(6)
      ...
      M[:, border3-1]=0
      F[border3-1,:] = 0
      K[np.ix_(border3-1, border3-1)] = sp.eye(border3.size)
      M[np.ix_(border3-1, border3-1)] = sp.eye(border3.size)
      ...
      for c in srch[0]:
       ...
       K[np.ix_(border3[np.arange(c,border3.size,6)]-1, border3[np.arange(c,border3.size,6)]-1)] -= sp.diags((K[np.ix_(border3[np.arange(c,border3.size,6)]-1, border3[np.arange(c,border3.size,6)]-1)]).diagonal()) - sp.eye(border3[np.arange(c,border3.size,6)].size)
        ...
        M[:, border3[::6] - 1] = 0
        F[border3[::6] - 1,:] = 0
        K[np.ix_(border3[::6] - 1, border3[::6] - 1)] = sp.eye(border3[::6].size)
        M[np.ix_(border3[::6] - 1, border3[::6] - 1)] = sp.eye(border3[::6].size)
        ...
        M[:, border3[1::6] - 1] = 0
        F[border3[1::6] - 1,:] = 0
        K[np.ix_(border3[1::6] - 1, border3[1::6] - 1)] = sp.eye(border3[1::6].size)
        M[np.ix_(border3[1::6] - 1, border3[1::6] - 1)] = sp.eye(border3[1::6].size)
        ...
        M[:, border3[2::6] - 1] = 0
        F[border3[2::6] - 1,:] = 0
        K[np.ix_(border3[2::6] - 1, border3[2::6] - 1)] = sp.eye(border3[2::6].size)
        M[np.ix_(border3[2::6] - 1, border3[2::6] - 1)] = sp.eye(border3[2::6].size)
        ...
        M[:, border3[3::6] - 1] = 0
        F[border3[3::6] - 1,:] = 0
        K[np.ix_(border3[3::6] - 1, border3[3::6] - 1)] = sp.eye(border3[3::6].size)
        M[np.ix_(border3[3::6] - 1, border3[3::6] - 1)] = sp.eye(border3[3::6].size)
        ...
        M[:, border3[4::6] - 1] = 0
        F[border3[4::6] - 1,:] = 0
        K[np.ix_(border3[4::6] - 1, border3[4::6] - 1)] = sp.eye(border3[4::6].size)
        M[np.ix_(border3[4::6] - 1, border3[4::6] - 1)] = sp.eye(border3[4::6].size)
    ...
    if boundaryconditions[3] == 1:
      ...
      if boundaryconditions[2] == 2:
       ...
       K[np.ix_(border4[np.arange(sz4-1,sz4-7,-1)]-1, border4[np.arange(sz4-1,sz4-7,-1)]-1)] = sp.eye(6)
      if boundaryconditions[0] == 2:
       K[border1[np.arange(0,6)] - 1, :] = 0
       K[:, border1[np.arange(0,6)]-1] = 0
       F[border1[np.arange(0,6)]-1,:] = 0
       K[np.ix_(border1[np.arange(0,6)] - 1, border1[np.arange(0,6)] - 1)] = sp.eye(6)
      ...
      K[:, border4-1]=0
      F[border4-1,:] = 0
      K[np.ix_(border4-1, border4-1)] = sp.eye(border4.size)
      M[np.ix_(border4-1, border4-1)] = sp.eye(border4.size)
      ...
      for c in srch[0]:
       ...
       K[np.ix_(border4[np.arange(c, border4.size, 6)] - 1, border4[np.arange(c, border4.size, 6)] - 1)] -= sp.diags((K[np.ix_(border4[np.arange(c, border4.size, 6)] - 1, border4[np.arange(c, border4.size, 6)] - 1)]).diagonal()) - sp.eye(border4[np.arange(c,border4.size,6)].size)
        ...
        M[:, border4[::6] - 1] = 0
        F[border4[::6] - 1,:] = 0
        K[np.ix_(border4[::6] - 1, border4[::6] - 1)] = sp.eye(border4[::6].size)
        M[np.ix_(border4[::6] - 1, border4[::6] - 1)] = sp.eye(border4[::6].size)
        ...
        M[:, border4[1::6] - 1] = 0
        F[border4[1::6] - 1,:] = 0
        K[np.ix_(border4[1::6] - 1, border4[1::6] - 1)] = sp.eye(border4[1::6].size)
        M[np.ix_(border4[1::6] - 1, border4[1::6] - 1)] = sp.eye(border4[1::6].size)
        ...
        M[:, border4[2::6] - 1] = 0
        F[border4[2::6] - 1,:] = 0
        K[np.ix_(border4[2::6] - 1, border4[2::6] - 1)] = sp.eye(border4[2::6].size)
        M[np.ix_(border4[2::6] - 1, border4[2::6] - 1)] = sp.eye(border4[2::6].size)
        ...
        M[:, border4[4::6] - 1] = 0
        F[border4[4::6] - 1,:] = 0
        K[np.ix_(border4[4::6] - 1, border4[4::6] - 1)] = sp.eye(border4[4::6].size)
        M[np.ix_(border4[4::6] - 1, border4[4::6] - 1)] = sp.eye(border4[4::6].size)
        ...
        M[:, border4[3::6] - 1] = 0
        F[border4[3::6] - 1,:] = 0
        K[np.ix_(border4[3::6] - 1, border4[3::6] - 1)] = sp.eye(border4[3::6].size)
        M[np.ix_(border4[3::6] - 1, border4[3::6] - 1)] = sp.eye(border4[3::6].size)
    ...
    K[np.ix_(np.arange(5,K.shape[0],6), np.arange(5,K.shape[0],6))]=sp.eye(int(K.shape[0] / 6))
    M[np.ix_(np.arange(5,M.shape[0],6), np.arange(5,M.shape[0],6))]=sp.eye(int(M.shape[0] / 6))
    ...
    Mb = np.delete(M.toarray(), fixed_borders - 1, 0)
    Mb = np.delete(Mb, fixed_borders - 1, 1)
    Kb = np.delete(K.toarray(), fixed_borders - 1, 0)
    ...
    if analysis_type == 1:
        U = sp.linalg.spsolve(sp.csr_matrix(K), sp.csr_matrix(F))
        return K, F, U

    elif analysis_type == 3:

        modes_number = int(input('Number of modes extracted ? '))
        w, modes = sp.linalg.eigs(Kb,modes_number,Mb)
...
def strain_calc_thick(X,T,u,v,w,thetax,thetay,th):
    ...
    thSTRAINxx = np.zeros((nX, 1))
    thSTRAINyy = np.zeros((nX, 1))
    thSTRAINxy = np.zeros((nX, 1))
    ...
    if th!=0:
        ...
        while i<nT :
            ...
            index1 = T[i,:]
            indexxx = np.arange(0, 7, 3)
            indexyy = np.arange(1, 8, 3)
            indexxy = np.arange(2, 9, 3)

            k = k_calc(X,T,w,thetax,thetay,i)

            e = STRAIN[[i, i + nT, i + 2 * nT]]
            e = np.concatenate((e,e,e),axis=0)

            thSTRAIN = e + th * k

            thSTRAINxx[index1] = thSTRAIN[indexxx]
            thSTRAINyy[index1] = thSTRAIN[indexyy]
            thSTRAINxy[index1] = thSTRAIN[indexxy]

            i+=1

    thSTRAINxx, thSTRAINyy, thSTRAINxy = thSTRAINxx[:,0], thSTRAINyy[:,0], thSTRAINxy[:,0]
...
def stress_calc(X,T,u,v,w,thetax,thetay,th,pos,Qprime,thickness):
    ...
    if th==0:
        ...
        stressxx, stressyy, stressxy = np.zeros((nX, 1)), np.zeros((nX, 1)), np.zeros((nX, 1))
        thSTRAINxx, thSTRAINyy, thSTRAINxy = strain_calc_thick(X, T, u, v, w, thetax, thetay, th)

        i=0
        while i<nX:
            thSTRAIN = np.array([[thSTRAINxx[i]], [thSTRAINyy[i]], [thSTRAINxy[i]]])
            thstress=np.dot(Qprime[3*ply:3*ply+3,:],thSTRAIN)
            thstress = thstress[:,0]

            stressxx[i] = thstress[0]
            stressyy[i] = thstress[1]
            stressxy[i] = thstress[2]
...
def show_stress(p,t,u, v, w, thetax, thetay, pos, Qprime, thickness):
    ...
    if yestr == 1:
        ...
        if th == 0:
            ...
            plt.gca().set_aspect('equal')
            if component == 1:
                plt.tricontourf(p[:, 0], p[:, 1], t, sxx, 500, cmap="jet")
            elif component == 2:
                plt.tricontourf(p[:, 0], p[:, 1], t, syy, 500, cmap="jet")
            else:
                plt.tricontourf(p[:, 0], p[:, 1], t, sxy, 500, cmap="jet")
...
def show_strain_thickness(p,t,u, v, w, thetax, thetay, pos, thickness, N):
    yes3 = int(input('plot Strain/thickness at a point ? Yes 1 / No 0 '))
    if yes3 == 1:
        ...
        fig11 = plt.figure()
        if component == 1:
            xplot = [thSTRAINxx1[pointu], thSTRAINxx2[pointu]]
            titledef = 'Strain exx (Ox) by lmainate thickness '
            labeldef = 'exx'
        elif component == 2:
            xplot = [thSTRAINyy1[pointu], thSTRAINyy2[pointu]]
            titledef = 'Strain eyy (Oy) by lmainate thickness '
            labeldef = 'eyy'
        else:
            xplot = [thSTRAINxy1[pointu], thSTRAINxy2[pointu]]
    ...
    if yes4 == 1:
        ...
        if component == 1:
            ...
            while i < N:
                ...
                stressused = [sxx1[pointu], sxx2[pointu]]
                xplot = stressused
            ...
            while i < N:
                sxx1, syy1, sxy1 = stress_calc(p, t, u, v, w, thetax, thetay, pos[i], pos, Qprime, thickness)
                sxx2, syy2, sxy2 = stress_calc(p, t, u, v, w, thetax, thetay, pos[i] + thickness[i], pos, Qprime, thickness)
                stressused = [syy1[pointu], syy2[pointu]]
                xplot = stressused
                yplot = [pos[i], pos[i] + thickness[i]]
            ...
            while i < N:
                sxx1, syy1, sxy1 = stress_calc(p, t, u, v, w, thetax, thetay, pos[i], pos, Qprime, thickness)
                sxx2, syy2, sxy2 = stress_calc(p, t, u, v, w, thetax, thetay, pos[i] + thickness[i], pos, Qprime, thickness)
                stressused = [sxy1[pointu], sxy2[pointu]]
                xplot = stressused
                yplot = [pos[i], pos[i] + thickness[i]]
...
def stress_LT_calc(X,T,u,v,w,thetax,thetay,th,pos,Qprime,thickness, angles):
    ...
    if th!=0:
        ...
        stress_L = np.zeros((nX, 1))
        stress_T = np.zeros((nX, 1))
        stress_LT = np.zeros((nX, 1))
...
def strain_LT_calc(X,T,u,v,w,thetax,thetay,th,pos,thickness, angles):
    ...
    if th!=0:
        ...
        while i<nX:
            ...
            strain_L = np.zeros((nX, 1))
            strain_T = np.zeros((nX, 1))
            strain_LT = np.zeros((nX, 1))

            straininter = np.array([[thSTRAINxx[i]],[thSTRAINyy[i]],[thSTRAINxy[i]]])
...
def show_deformation_LT(p,t,u, v, w, thetax, thetay,pos,thickness, angles):
    ...
    if yes == 1:
        ...
        if th == 0:
            ...
            fig13 = plt.figure()
            if component == 1:
                plt.tricontourf(p[:, 0], p[:, 1], t, strain_L, 500, cmap="jet")
            elif component == 2:
                plt.tricontourf(p[:, 0], p[:, 1], t, strain_T, 500, cmap="jet")
            else:
                plt.tricontourf(p[:, 0], p[:, 1], t, strain_LT, 500, cmap="jet")
...
def Hoffman(SLLt, SLLc, STTt, STTc, SLT, X,T,u,v,w,thetax,thetay,th,pos,Qprime,thickness, angles):
    ...
    Hoffman_stress = np.zeros((N,1))

    i=0
    while i<N:

        Hoffman_stress[i] = ((stress_L[i]**2)/(SLLt*SLLc)) + ((stress_T[i]**2)/(STTt*STTc)) - ((stress_T[i]*stress_L[i])/(SLLt*SLLc)) + ((SLLc - SLLt)/(SLLt*SLLc))*stress_L[i] + ((STTc - STTt)/(STTt*STTc))*stress_T[i] + (stress_LT[i]**2)/(SLT**2)
        i+=1
...
def show_Hoffman_stress(p,t,u,v,w,thetax,thetay,pos,Qprime,thickness, angles):
    ...
    if yeshoffstr == 1:
        ...
        if th == 0:
            ...
            plt.tricontourf(p[:, 0], p[:, 1], t, Hoffman_stress, 500, cmap="jet")
...
def get_type():
    analysis_type = int(input('Static 1 / Dynamic 2 / Frequency 3 : '))
